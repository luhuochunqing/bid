...
- 验证命令：`npm run agent:git-safe-check`（或 `bash scripts/check-git-wrapper.sh`）。
...
2. **CI 速度优化（减少队列等待）**
   ...
   - **强烈建议**：每次 push 前必须本地 `npm run ci:local:quick` + `npm run agent:git-safe-check` + `npm run agent:lock-check:changed` + `npm run build`。只有本地全绿才 push，远程只做"确认"而非"发现问题"。
...
- specs/005-org-sdk-integration/plan.md (Org SDK Integration)
- specs/016-project-archive-stage-timestamps/plan.md (Project Archive Stage Timestamps)
<!-- SPECKIT END -->
