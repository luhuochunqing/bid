...
class AsyncConfigTest {
    ...
    void performanceBundleExportExecutor_queueFull_throwsRejectedExecution() throws InterruptedException {
        Executor executor = asyncConfig.performanceBundleExportExecutor();
        ThreadPoolTaskExecutor poolExecutor = (ThreadPoolTaskExecutor) executor;

        // 用 CountDownLatch 阻塞工作线程，避免 Thread.sleep(5000) 拖慢测试套件。
        // latch 不 countDown，工作线程一直阻塞直到 shutdownNow 中断。
        ...
        try {
            ...
            // 清理：shutdownNow 会中断所有阻塞在 latch.await() 的工作线程
            java.util.concurrent.ExecutorService underlyingPool = poolExecutor.getThreadPoolExecutor();
            underlyingPool.shutdownNow();
            assertTrue(underlyingPool.awaitTermination(5, TimeUnit.SECONDS),
                    "线程池应在 5 秒内终止");
        }
    }
}
