-- Input: V1166__align_tender_entry_schema_with_fallback.sql
-- ================================================================
-- U1166: 回滚 V1166 — 恢复 tender.entry schema 到 V1007 状态
-- 说明：恢复 version=2 与 V1007 设置的 schema_json（含 tenderAgency/
--       contactName/contactPhone 等旧字段 key，customerType 7 选项，
--       projectType required=false，tenderInfo maxLength=5000，
--       以及 sourcePlatform/budget/crmOpportunityId 扩展字段）
-- 警告：回滚后业务页 ManualTenderDialog 仍会走 fallback 表单渲染
--       （因为 V1007 schema 与 fallback 字段 key 不一致），不会
--       影响业务功能，只是 DynamicFormRenderer 数据映射问题会重现
-- ================================================================
...
-- ----------------------------------------------------------
-- 2. 恢复 tender.entry schema 到 V1007 状态
-- ----------------------------------------------------------
