...
| `service/ProjectDocumentWorkflowService.java` | Service | 项目文档查询、元数据创建和删除编排，调用文档绑定边界。文档上传/删除时通过 `DocumentChangeNotificationService.notifyDocumentChanged` 触发 `DOCUMENT_CHANGE` 通知项目团队成员（蓝图 §消息中心-系统通知 序号 5）。当前产品层无文档修改 API，`DocumentOperationType.MODIFY` 保留为占位 |
