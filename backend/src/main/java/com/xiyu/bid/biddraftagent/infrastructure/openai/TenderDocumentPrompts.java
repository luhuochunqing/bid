...
final class TenderDocumentPrompts {
    ...
    static String buildTenderIntakePrompt(DocumentAnalysisInput input, DocumentChunk chunk) {
        ...
        return """
                ...
                - purchaserName：招标主体名称（业务约定的 7 种 + 兼容历史口径 1 种，共 8 种标签：%s）。常见位置：文本开头的公司全称、"招标人：XXX"、"招标单位：XXX"、"采购人：XXX"、"采购单位：XXX"、"项目单位：XXX"、"实施单位：XXX"、"需求单位：XXX"、"业主单位：XXX"后的名称；任一标签命中即填入 purchaserName，不得因"该标签未在口径中列出"而丢弃。注意"XX（集团）有限责任公司"是完整公司名。
