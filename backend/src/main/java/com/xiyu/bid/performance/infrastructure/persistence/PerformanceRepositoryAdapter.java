...
public class PerformanceRepositoryAdapter implements PerformanceRepository {
    ...
    public PerformanceRecord save(PerformanceRecord record) {
        ...
        for (var att : record.attachments()) {
            var attEntity = toAttEntity(att, saved.getId());
            attRepository.save(attEntity);
        }
    ...
    public List<PerformanceRecord> findAll(PerformanceSearchCriteria criteria, PerformanceAlertConfig config) {
        ...
        if (criteria.customerType() != null && !criteria.customerType().isBlank()) {
            customerType = CustomerType.valueOf(criteria.customerType().toUpperCase());
        }
        ProjectType projectType = null;
        if (criteria.projectType() != null && !criteria.projectType().isBlank()) {
            projectType = ProjectType.valueOf(criteria.projectType().toUpperCase());
        }
        ...
        return jpaRepository.findByCriteria(
                ...
                criteria.status(),
                criteria.territory(),
                criteria.signingDateStart(),
                criteria.signingDateEnd(),
                criteria.expiryDateStart(),
                criteria.expiryDateEnd(),
                criteria.hasBidNotice(),
                criteria.projectManagerKeyword(),
                today,
                expiringDateSOE,
                expiringDateDefault
    ...
    @Override
    @Transactional(readOnly = true)
    public PagedResult<PerformanceRecord> findAllPageable(PerformanceSearchCriteria criteria, PerformanceAlertConfig config, int pageNumber, int pageSize) {
        LocalDate today = LocalDate.now();
        LocalDate expiringDateSOE = today.plusDays(config.alertDaysSoe());
        LocalDate expiringDateDefault = today.plusDays(config.alertDaysDefault());
        ...
        Page<PerformanceRecordEntity> page = jpaRepository.findByCriteriaPageable(
                criteria.keyword(),
                customerType,
                projectType,
                criteria.status(),
    ...
    private PerformanceRecord toDomain(PerformanceRecordEntity e) {
        ...
        return new PerformanceRecord(
                ...
                e.getSigningDate(),
                e.getExpiryDate(),
                e.getTotalExpiryDate(),
                e.getContactPerson(),
                e.getContactInfo(),
                e.getTerritory(),
                e.getCustomerAddress(),
                e.getXiyuProjectManager(),
                e.getMallWebsiteUrl(),
                e.isHasBidNotice(),
                e.getRemarks(),
                atts,
