 ...
 *
 * <p>对标 {@code WarehouseExportTaskStateService}，提供独立事务的状态更新方法，
 ...
 * </ul>
 *
 * @see com.xiyu.bid.warehouse.application.WarehouseExportTaskStateService
 */
@Service
...
public class PerformanceBundleExportTaskStateService {
    ...
    private final PerformanceExportTaskRepository repo;
    ...
    public Long createTask(String filterSnapshot, Long operatorId) {
        PerformanceExportTaskEntity task = PerformanceExportTaskEntity.builder()
     ...
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public PerformanceExportTaskEntity complete(PerformanceExportCompletion ctx) {
        LocalDateTime now = LocalDateTime.now();
        PerformanceExportTaskEntity task = repo.findById(ctx.taskId()).orElseThrow();
        task.setStatus(ExportStatus.COMPLETED);
        task.setTotalCount(ctx.totalCount());
        task.setStoredFilePath(ctx.filePath());
        task.setDownloadUrl(DOWNLOAD_URL_PREFIX + ctx.taskId() + "/download");
