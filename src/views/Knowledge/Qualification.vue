      ...
      <el-table :data="qualifications" v-loading="loading" style="width:100%" @row-click="handleRowClick">
        ...
        <el-table-column label="操作" width="120" fixed="right" align="center">
          <template #default="scope">
            ...
            <el-button v-if="scope.row.fileUrl && !canManageQualification" link type="primary" size="small" @click.stop="handleDownload(scope.row)">下载</el-button>
          </template>
        </el-table-column>
      </el-table>
...
const MANAGED_OR_VIEW_ROLES = ['admin_staff', 'bid_admin', 'bid_lead', 'bid_specialist']

const canManageQualification = computed(() => MANAGED_ROLES.includes(userStore.userRole))
const canViewQualification = computed(() => MANAGED_OR_VIEW_ROLES.includes(userStore.userRole))
...
const borrowDialogVisible = ref(false); const currentProjectId = ref(''); const checkingBorrow = ref(false); const activeQualId = ref(null)
...
const handleDownload = (row) => { activeQualId.value = row.id; currentProjectId.value = ''; borrowDialogVisible.value = true }
const confirmBorrowCheck = async () => {
  if (!currentProjectId.value) { ElMessage.warning('请输入关联的项目ID'); return }
  checkingBorrow.value = true
  try {
    const { data } = await http.get(`/api/qualification/${activeQualId.value}/check-borrow`, { params: { projectId: currentProjectId.value } })
    if (data.allowed) { ElMessage.success('审计通过'); borrowDialogVisible.value = false }
    else ElMessage.error('被拦截：' + (data.reason || '未绑定借阅流程'))
  } catch (error) {
    if (error.response?.status === 403) ElMessage.error('权限不足')
    else ElMessage.error('网络异常')
  } finally { checkingBorrow.value = false }
}
