<template>
  <el-card class="case-card" shadow="hover" :class="{ 'is-pinned': caseData.pinned }" style="cursor:pointer" @click="$emit('view-detail', caseData.caseId)">
    <div class="case-card-header" @click.stop>
      <div class="title-row">
        <el-tag v-if="caseData.pinned" size="small" type="warning" effect="dark" class="pinned-badge">置顶</el-tag>
        <el-tag v-if="caseData.score != null" size="small" :type="caseData.score >= 85 ? 'danger' : 'primary'" class="score-badge">{{ caseData.score }}%</el-tag>
        <el-tag v-if="caseData.scoreLabel" size="small" type="danger" effect="dark">{{ caseData.scoreLabel }}</el-tag>
      ...
      <div class="tag-row mt-2">
        ...
        <el-tag v-if="caseData.bidResult" size="small" :type="caseData.bidResult === 'WON' ? 'success' : 'info'" effect="plain" class="ml-1">
          {{ caseData.bidResult === 'WON' ? '中标' : '未中标' }}
        </el-tag>
      </div>
    </div>
    <div class="case-card-body" @click.stop>
    ...
    <div class="case-card-footer">
      ...
      <div class="action-buttons">
        <el-button v-if="canManage" size="small" @click.stop="$emit('toggle-pin', caseData)">
          {{ caseData.pinned ? '取消置顶' : '置顶' }}
        </el-button>
        <el-button size="small" type="primary" link @click.stop="$emit('view-detail', caseData.caseId)">详情</el-button>
        <el-button size="small" type="success" @click.stop="$emit('reuse', caseData)"><el-icon style="vertical-align:-.2em;margin-right:4px"><CopyDocument /></el-icon>复用</el-button>
        <el-button v-if="canManage" size="small" type="danger" plain @click.stop="$emit('off-shelf', caseData)">下架</el-button>
      </div>
    </div>
  </el-card>
</template>
...
defineEmits(['view-detail', 'reuse', 'toggle-pin', 'off-shelf'])
