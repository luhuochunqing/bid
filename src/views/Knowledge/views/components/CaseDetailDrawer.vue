<template>
  <el-drawer v-model="internalVisible" :title="caseData?.scoringPointTitle || caseData?.scoringTitle || '案例详情'" size="560px" destroy-on-close>
    <div v-if="caseData" class="drawer-container" v-loading="loading">
      <div class="section-title">案例元信息</div>
      ...
      <div class="section-title mt-4">原始招标需求</div>
      <div class="content-box requirement-box">{{ caseData.requirementRaw }}</div>
      <div class="section-title mt-4 flex justify-between align-center">
        <span>推荐应答片段</span>
      </div>
      <div class="word-count-hint">
        共 {{ (caseData?.responseText || '').length }} 字
      </div>
      ...
      <div class="section-title mt-6">复用历史</div>
...
.related-case-item .rel-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; }
...
.related-case-item .rel-summary { font-size: 12px; color: var(--el-text-color-regular); margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
