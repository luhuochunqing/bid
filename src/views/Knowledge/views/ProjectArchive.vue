<template>
  <div class="project-archive-container">
    ...
    <el-card class="filter-card kb-filter-card">
      <template #header>
        <div class="card-header-title"><el-icon><Files /></el-icon><span>{{ activeView === 'file' ? '文档分类下载' : '项目档案台账' }}</span></div>
      </template>
      <el-form :inline="true" :model="filters" class="search-form">
        <el-form-item label="项目名称"><el-input v-model="filters.projectName" placeholder="请输入项目名称" clearable style="width: 180px" /></el-form-item>
        <el-form-item label="文档分类">
          <el-select v-model="filters.categories" placeholder="选择分类" multiple clearable collapse-tags collapse-tags-tooltip style="width: 200px">
            <el-option label="招标文件" value="TENDER" /><el-option label="标书文件" value="BID" /><el-option label="开标一览表" value="OPEN_LIST" />
            <el-option label="中标通知书" value="WIN_NOTICE" /><el-option label="保证金银行回单" value="DEPOSIT_RECEIPT" /><el-option label="其他" value="OTHER" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="项目状态">
          <el-select v-model="filters.projectStatus" placeholder="选择状态" multiple clearable collapse-tags collapse-tags-tooltip style="width: 200px">
            ...
            <el-option label="已中标" value="WON" /><el-option label="未中标" value="LOST" /><el-option label="流标" value="FAILED" /><el-option label="弃标" value="ABANDONED" />
          </el-select>
        </el-form-item>
...
const buildQueryParams = (view = 'project') => {
  const params = buildFilterParams()
  if (view === 'file') {
    params.page = filePage.value - 1
    params.size = filePageSize.value
  } else {
...
const loadFileData = async () => {
  loading.value = true
  try {
    const res = await httpClient.get('/api/archive/files', { params: buildQueryParams('file') })
    fileTableData.value = res.content || []
    fileTotalElements.value = res.totalElements || 0
  } catch (e) { ElMessage.error('加载文档分类列表失败'); console.error(e) }
...
const handleFilePageChange = (p, size) => {
  filePage.value = p
...
const handleExportExcel = async () => {
  try {
    const blob = (await httpClient.post('/api/archive/export-excel', buildFilterParams(), { responseType: 'blob' })).data
...
const handleExportZip = async () => {
  try {
    const blob = (await httpClient.post('/api/archive/export-zip', buildFilterParams(), { responseType: 'blob' })).data
...
const handleDownloadFileByItem = async (row) => {
  try {
    const res = await httpClient.get(`/api/archive/files/${row.fileId}/download`, { responseType: 'blob' })
    downloadBlob(res.data, row.fileName || '下载文件', res.data.type || 'application/octet-stream')
  } catch { ElMessage.warning('文件下载失败') }
}
