      ...
      <el-row v-else :gutter="20" class="card-grid">
        <el-col v-for="item in cases" :key="item.id || item.caseId" :xs="24" :sm="12" :md="8" :lg="6" class="grid-col">
          <CaseCard :case-data="item" :can-manage="canManage"
            @view-detail="viewDetail" @reuse="handleReuse" @toggle-pin="handleTogglePin" @off-shelf="handleOffShelfClick(item)" />
        </el-col>
      </el-row>
...
const loadReuseHistory = async (caseId) => {
  try { const references = await casesApi.getReferenceRecords(caseId); reuseHistory.value = Array.isArray(references?.data) ? references.data.map(r => ({ time: r.referencedAt || r.createdAt || '', referencedByName: r.referencedByName || '用户', referencedProjectName: r.sourceProjectName || '' })) : []
