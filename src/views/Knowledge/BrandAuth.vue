    ...
    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      <el-tab-pane label="原厂授权" name="manufacturer">
        <div class="tab-toolbar">
          <el-button type="primary" @click="openCreate"><el-icon><Plus /></el-icon> 新增原厂授权</el-button>
          ...
          <el-table :data="records" stripe @row-click="openDetail">
            ...
            <el-table-column label="操作" width="180" fixed="right" align="center">
              <template #default="{row}">
                ...
                <el-button v-if="row.status !== 'REVOKED'" type="primary" link size="small" @click.stop="openEdit(row)">编辑</el-button>
                <el-button v-if="row.status !== 'REVOKED'" type="danger" link size="small" @click.stop="showRevokeDialog(row)">作废</el-button>
              </template>
            </el-table-column>
          </el-table>
      ...
      <el-tab-pane label="代理商授权" name="agent">
        <el-empty description="代理商授权功能即将上线" />
      </el-tab-pane>
    </el-tabs>

    <BrandAuthFormDrawer v-model="formVisible" :initial-data="editData" @save="handleSave" @close="editData=null" />
...
const filters = reactive({ productLines:[], brandId:'', brandName:'', importDomestic:'', manufacturerName:'', statuses:[], keyword:'' })
...
const loadData = async () => {
  loading.value = true
  try {
    const { data } = await brandAuthApi.getList({ ...filters, page: page.value-1, size: pageSize.value })
    records.value = data?.content || []; total.value = data?.totalElements || 0
  } catch { ElMessage.error('加载失败') } finally { loading.value = false }
}

const resetFilters = () => {
  Object.assign(filters, { productLines:[], brandId:'', brandName:'', importDomestic:'', manufacturerName:'', statuses:[], keyword:'' })
...
const onTabChange = (t) => { if (t === 'manufacturer') loadData() }
onMounted(loadData)
</script>
