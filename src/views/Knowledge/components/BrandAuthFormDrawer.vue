...
const formRules = {
  ...
  authStartDate: [{ required: true, message: '请选择开始时间' }],
  authEndDate: [{ required: true, message: '请选择结束时间' }]
}
...
function getEmptyForm() {
  return {
    ...
    authDocFileList: [], supplementaryFileList: []
  }
}
...
const handleSubmit = () => {
  const f = form.value
  if (!f.productLine || !f.brandId || !f.brandName || !f.manufacturerName || !f.authStartDate || !f.authEndDate) {
