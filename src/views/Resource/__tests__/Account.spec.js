...
// ── CO-400：列表 N+1 detail 请求分批并发，避免触发 429 限流 ──────────────────

describe('Account.vue — loadAccounts 分批并发拉取详情', () => {
  beforeEach(() => {
    setActivePinia(createPinia())
    resourcesApiMock.accounts.getList.mockReset()
    resourcesApiMock.accounts.getDetail.mockReset()
  })
  ...
  it('列表加载时按 DETAIL_CONCURRENCY 批次并发调 getDetail，不一次性 burst', async () => {
    ...
    resourcesApiMock.accounts.getDetail.mockImplementation(async (id) => {
      inFlight += 1
      maxInFlight = Math.max(maxInFlight, inFlight)
      await new Promise((resolve) => setTimeout(resolve, 0))
      inFlight -= 1
      return { data: { ...manyAccounts.find((a) => a.id === id), password: 'secret' } }
    })

    const wrapper = mountAccount()
    // 等待所有 batch 完成：每批内部 await Promise.all，再进入下一批。
    await flushPromises()
    await new Promise((resolve) => setTimeout(resolve, 10))
    await flushPromises()

    expect(wrapper.vm.accounts).toHaveLength(6)
    expect(resourcesApiMock.accounts.getDetail).toHaveBeenCalledTimes(6)
    // DETAIL_CONCURRENCY 当前为 2，与 BAR 站点列表保持一致（PR !2000）。
