  ...
  <div class="closure-stage">
    ...
    <div class="form-section">
      ...
      <template v-if="preview?.hasDeposit">
        ...
        <!-- 只读：已提交的退回情况 -->
        <template v-if="!canFillDeposit && preview?.depositReturnStatus && preview?.depositReturnStatus !== 'NA'">
    ...
    <div class="form-section">
      <div class="form-section-title">
        ...
        <div class="section-actions">
          ...
          <el-tooltip v-if="preview?.reviewStatus === 'APPROVED' && isBidManager" :content="precipitateTooltip" :disabled="precipitateReady" placement="top" effect="dark">
      ...
      <div class="field-item full-width">
        ...
        <div v-else class="summary-readonly" v-html="form.projectSummary || '<span class=\'text-gray\'>(暂无项目总结)</span>'"></div>
      </div>
    </div>
...
async function checkPrecipitationReadiness() {
  try {
    ...
    const data = resp?.data || resp; precipitateReady.value = data.canPrecipitate
    if (!data.canPrecipitate && data.missingItems?.length) precipitateTooltip.value = data.missingItems.join('；')
  } catch (e) { precipitateReady.value = false; precipitateTooltip.value = '无法检查前置条件' }
}
