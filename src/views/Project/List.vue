    ...
    <el-card class="table-card b2b-section-card" shadow="never">
      ...
      <div v-else class="table-wrapper">
        <el-table :data="filteredProjects" stripe>
          ...
          <el-table-column prop="sourceModule" label="来源平台" width="110" v-if="columnVisible.sourceModule">
            <template #default="{ row }">{{ sourceText(row.sourceModule) }}</template>
          </el-table-column>
          ...
          <el-table-column label="创建时间" width="105" sortable="custom" v-if="columnVisible.createTime">
            <template #default="{ row }">{{ formatDate(row.createdAt) }}</template>
          </el-table-column>
          <el-table-column label="开标时间" width="105" sortable="custom" v-if="columnVisible.bidOpenTime">
          ...
          <el-table-column label="客户营收（万）" width="120" sortable="custom" v-if="columnVisible.revenue">
...
const { loading, error, matchedProjects, filteredProjects, pagination, handleSizeChange, handlePageChange, resetPage } =
...
.table-card :deep(.el-card__header) { padding: 14px 20px; }
