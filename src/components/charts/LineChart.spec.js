...
describe('LineChart', () => {
  it('挂载后 init 并 setOption 初始 option', () => {
    const opt = option()
    const wrapper = mount(LineChart, { props: { option: opt } })
    expect(instances.at(-1).setOption).toHaveBeenCalledWith(opt)
    wrapper.unmount()
  })

  it('option 变化触发 setOption(option, true)', async () => {
    const wrapper = mount(LineChart, { props: { option: option() } })
    await wrapper.setProps({ option: { series: [{ type: 'line', data: [9] }] } })
  ...
  it('转发图表 click 事件为 chart-click', () => {
    ...
    clickCb({ dataIndex: 1 })
    expect(wrapper.emitted('chart-click')?.[0]).toEqual([{ dataIndex: 1 }])
    wrapper.unmount()
  })
