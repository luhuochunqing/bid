...
| projectManagerEmployeeId | String | 否 | 项目负责人工号（最长 32 字符，v3.10 新增）。与 projectManagerName 配套，工号全局唯一用于精确反查本地 user_id；同时传入时工号优先，姓名仅作校验；工号未命中或为空时回落姓名匹配（重名仍跳过避免误绑）| "E001234" |
...

{
  ...
  "projectManagerEmployeeId": "E001234",
  "department": "技术部",
  "creatorName": "李四",
...
| projectManagerName | String | 否 | 项目负责人姓名（最长 100 字符，v3.7 新增；传入后同步反查本地 user_id 落库） | "张三" |
| projectManagerEmployeeId | String | 否 | 项目负责人工号（最长 32 字符，v3.10 新增）。与 projectManagerName 配套，工号优先反查本地 user_id；工号未命中或为空时回落姓名匹配（重名跳过避免误绑） | "E001234" |
| department | String | 否 | 项目部门（最长 100 字符，v3.7 新增） | "技术部" |
...
| v3.10 | 2026-07-20 | **§3.2 标讯创建 / §3.3 标讯修改 新增 `projectManagerEmployeeId`（项目负责人工号）字段：** 与 `projectManagerName` 配套，工号全局唯一用于精确反查本地 user_id；同时传入时工号优先，姓名仅作校验（不一致仅告警不阻断）；工号未命中或为空时回落姓名匹配（重名仍跳过避免误绑）。解决 v3.7 仅按姓名反查时同名员工无法绑定 user_id 的问题 |
