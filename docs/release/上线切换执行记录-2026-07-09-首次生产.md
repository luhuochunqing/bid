...
| 4.3.4 P2 验证 | Trae Agent | 22:00 | 22:30 | ⚠️ AI 配置读取正常（Key 未配置）+ 项目列表 + CRM 鉴权 |
...
**说明**：
1. 整体状态 DOWN 仅因 sidecar 组件不可达。sidecar 是可选组件，`fallbackAvailable: true` 表示有降级方案。后端核心功能全部正常。
2. **aiProvider 风险**：组件状态为 UP（健康检查通过），但 `apiKeyConfigured=false` 表示**生产环境 AI API Key 实际未配置**。当前 AI 相关功能（如标书生成、智能补全）调用时会因鉴权失败而不可用。需在试运行期内配置真实 API Key 后复测。
...
| AI 配置 | ⚠️ provider=deepseek, model=deepseek-chat | **apiKeyConfigured=false**（Key 未注入），见 §8.4 |
...
| 验证项 | 结果 | 备注 |
|--------|------|------|
| AI 连接测试 | ⚠️ 未实测 | `apiKeyConfigured=false`，Key 未注入，无法实测 AI 接口；见 §8.4 遗留问题 |
...
| 全量人员同步 | ✅ 8528 用户入库 | 同步 run 报告：源端 11,200 用户，写入成功 11,053，跳过 147（数据质量问题：98 手机号为空 + 47 邮箱重复 + 2 其他），users 表最终落库 8,528（含本地历史 169 + OSS 同步 8,359） |
...
| 问题 | 影响 | 处置 |
|------|------|------|
| **AI API Key 未配置**（`apiKeyConfigured=false`） | **AI 功能不可用**（标书生成、智能补全等） | **需客户运维或开发团队配置真实 deepseek API Key 后复测**；试运行期前必须解决 |
