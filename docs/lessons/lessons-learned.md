...
## 87. 结构化标签字段死磕 Prompt 不如正则兜底——purchaserName 复合 bug 的上游修复之一（2026-07-29 / PR !2222）

> ⚠️ 本条原称"最终根治"，实际只是 6 次修复中的第 2 次上游修复。下游还有持久化层（第 88 条）、前端 mapping（第 89 条）、部署遗漏（第 90 条）等问题。详见第 90 条"为什么修复了 6 次才成功"的反思。
...
## 88. 持久化层"只在空值时更新"策略让上游修复全部失效——purchaserName 复合 bug 的下游根因之一（2026-07-30 / PR !2226）

> ⚠️ 本条原称"四次修复的零号病人"，实际只是 6 次修复中的第 4 次下游修复。前端 mapping（第 89 条）和部署遗漏（第 90 条）仍未解决。详见第 90 条"为什么修复了 6 次才成功"的反思。
...
## 89. 前端字段映射缺 purchaserName 导致正则兜底修复后表单仍显示空——purchaserName 复合 bug 的前端根因之一（2026-07-30 / PR !2227）

> ⚠️ 本条原称"五次修复的真正零号病人"，实际只是 6 次修复中的第 5 次前端修复。第 6 次部署遗漏（第 90 条）才让修复真正生效。详见第 90 条"为什么修复了 6 次才成功"的反思。
...
## 90. purchaserName 复合 bug 六次修复才成功的真正根因——过早宣告终结 + 复合 bug 认知缺失 + 端到端验证缺失 + 部署遗漏（2026-07-30 / 第 115 次测试环境部署）

### 事故背景

张家口银行招标文件"招标主体识别"bug，从 2026-07-29 到 2026-07-30 经历 **6 次修复**才最终解决：

| 修复次数 | 改动层 | PR | 结果 |
|---------|--------|-----|------|
| 第 1 次 | 归一化字符集 + Prompt | !2214 + !2218 | 失败（AI 仍漏抽） |
| 第 2 次 | 正则兜底（AI 空值才触发） | !2222 | 失败（兜底未触发） |
| 第 3 次 | 正则优先覆盖 AI + 候选文本优化 | !2224 | 失败（持久化层丢弃新值） |
| 第 4 次 | 持久化层覆盖策略 | !2226 | 失败（前端 mapping 缺字段） |
| 第 5 次 | 前端 mapping 加 purchaserName | !2227 | 失败（部署遗漏） |
| 第 6 次 | 部署最新 main（含 !2227） | 第 115 次部署 | ✅ 成功 |

### 真正根因（四大元凶）

#### 元凶 1：复合 bug 认知缺失

**核心误区**：把"前端表单显示错误"当成"单一根因 bug"来诊断，实际是 **5 个独立 bug 叠加**：

1. 归一化字符集不全（第 86 条）
2. AI 漏抽且无兜底（第 87 条）
3. 候选文本构建缺陷（第 88 条上下文）
4. 持久化层只在空值时更新（第 88 条）
5. 前端 mapping 缺 purchaserName（第 89 条）

**为什么这 5 个 bug 之前没被发现**：它们各自独立存在时，AI 偶然能正确识别（前面层用错误值"祥安招标代理有限公司"恰好填满字段，掩盖了下游所有问题）。只有当上游修复让 purchaserName 正确、tenderAgency 变空时，下游问题才暴露。

**必然性证明**：
```
1. AI 误识别 purchaserName = tenderAgency = "祥安招标代理有限公司"
   → 前端 mapping 读 tenderAgency 填表单 → 显示错误值（但非空）
   → 用户感知"识别错了"
2. 修上游让 purchaserName 正确、tenderAgency 变空
   → 前端 mapping 缺 purchaserName，读不到任何值
   → 显示空值
   → 用户感知"反而更差了"
3. 修前端 mapping 让 purchaserName 能被读取
   → 但部署遗漏，前端代码没更新
   → 仍显示空值
4. 重新部署含前端修复的代码
   → ✅ 显示正确值
...
**关键认知**：用户感知的"招标主体识别错误"是一个**症状**，背后是 5 个独立 bug 叠加。每次修复只解决一个，剩余 bug 通过"用户反馈变差了"暴露出来。

#### 元凶 2：过早宣告终结（教训沉淀的元 bug）

第 87/88/89 条教训的标题分别使用了"最终根治"/"零号病人"/"真正零号病人"等绝对化表述，但每次都还有下一个问题。这种"宣告终结"的标题传递了错误的信心，导致：

- 修复者认为问题已解决，不再继续验证
- 部署后只验证"修过的那一层"，不验证端到端
- 教训沉淀过早——bug 还没真正解决就写教训，教训本身就不完整

**反思**：教训沉淀应该在**用户验证通过后**才写，而不是"自己改完代码就写"。第 87/88/89 条教训的标题已经全部修改为"复合 bug 的上游/下游/前端根因之一"，去掉"最终根治"/"零号病人"等绝对化表述。

#### 元凶 3：端到端验证缺失

**6 次修复中没有任何一次做了完整端到端验证**：

| 修复次数 | 验证范围 | 漏掉的环节 |
|---------|---------|-----------|
| 第 1-3 次 | 后端日志 purchaserName 有值 | 前端表单是否显示 |
| 第 4 次 | 后端持久化层单元测试 | 前端 mapping 是否读取 |
| 第 5 次 | 前端单元测试 | 部署的代码版本是否包含修复 |
| 第 6 次 | 端到端（部署+前端显示） | ✅ 完整 |

**正确的端到端验证清单**：
1. 用户上传 PDF → 前端发起 API 请求
2. 后端接收 → sidecar 提取 → 候选文本构建 → AI 分析 → 正则兜底
3. 后端 mergeAndMap 写入 extractedData.purchaserName
4. 后端返回响应 → 前端接收
5. 前端 applyParsedFields 读取 extractedData.purchaserName → 填入 form.purchaser
6. 前端表单显示"张家口银行股份有限公司"
7. 用户肉眼确认

**漏掉任何一环，都可能让修复"看起来成功但实际失败"**。

#### 元凶 4：部署遗漏（PR 合并 ≠ 已部署）

第 6 次修复失效的根因是**部署遗漏**：

- 第 114 次部署时间：2026-07-30 11:53:09 CST（基线 `268736e12`，仅含 PR !2226）
- PR !2227 合并时间：2026-07-30 12:22:08 CST（前端 mapping 修复）
- 时间差：部署后 29 分钟才合并 PR !2227
- 必然结果：测试环境前端代码仍是旧版本，前端 mapping 缺 purchaserName

**根因**：部署基线选择的是"上一版 main HEAD"而非"最新 main HEAD"。PR 合并后没有立即触发新部署，也没有验证"部署的代码版本是否包含所有修复"。
...
| 根因 | 体现 |
|------|------|
| **根因 1：追症状不追根因** | 5 次修复都在不同层（Prompt→正则→候选文本→持久化→前端），但每次都以为找到了"最终根治"，实际只是复合 bug 的一个组成部分 |
| **根因 2：外部配置覆盖代码修复** | 第 6 次的"部署遗漏"——PR 合并了但没部署，相当于"代码修复了但环境未更新" |
| **根因 3：环境差异掩盖问题** | 本地测试 vs 测试环境部署的代码版本不一致（本地有 PR !2227，测试环境没有） |
| **根因 4：测试只测修过的函数** | 5 次修复都只测自己改的那一层（后端测后端、前端测前端），从不测端到端 |
| **根因 7：盲目相信"已修复"** | 5 次教训标题都用"最终根治"/"零号病人"宣告终结，但每次都还有下一个问题 |

### 核心教训

1. **复合 bug 必须用端到端验证才能根治**：当 bug 由多个独立问题叠加时，单层修复必然失败。必须从用户操作 → 前端表单 → API → 后端 → 数据库 → API 响应 → 前端表单回填全链路验证，才能确认真正解决。

2. **教训沉淀不能过早**：教训应该在**用户验证通过后**才写，而不是"自己改完代码就写"。过早沉淀的教训会传递错误信心（"最终根治"），让修复者停止继续验证。本条教训的所有前序条目（87/88/89）都已修改标题，去掉绝对化表述。

3. **"用户反馈变差了"是关键信号**：第 4 次修复后用户从"错误值"变成"空值"，这是复合 bug 的典型信号——说明修复影响了某个中间环节，应该立即做端到端排查，而不是继续在当前层打补丁。

4. **PR 合并 ≠ 已部署**：部署基线必须确认包含所有修复 commit。PR 合并后应立即部署，或在部署报告中明确记录"本次部署不含 PR !XXX，需后续部署"。

5. **"过早宣告终结"是反复修复的元 bug**：每次以为找到了"最终根治"/"零号病人"，实际只是复合 bug 的一个组成部分。正确的表述应该是"X 层根因之一"，留有余地继续排查其他层。

### 操作规范

1. **复合 bug 必须做端到端验证**：用户操作 → 前端表单 → API → 后端 → 数据库 → API 响应 → 前端表单回填，7 个环节全验证
2. **教训沉淀时机**：用户验证通过后才写教训，自己改完代码只能写"修复笔记"，不能写"最终根治"
3. **教训标题禁用绝对化表述**：禁用"最终根治"/"零号病人"/"真正零号病人"，改用"X 层根因之一"
4. **部署后必须验证代码版本**：`cat /opt/xiyu-bid/deployed-release.json` 确认 releaseId 包含所有修复 commit
5. **PR 合并后立即部署或标记未部署**：不能假设"PR 合并了就生效了"
6. **用户反馈"变差了"时立即启动端到端排查**：不要继续在当前层打补丁

### SOP 补充：部署版本验证步骤

部署后必须执行以下验证（加入 xiyu-server-deploy skill）：

```bash
# 1. 确认部署的 releaseId 包含所有修复 commit
ssh jetty@${TARGET_HOST} 'cat /opt/xiyu-bid/deployed-release.json | grep releaseId'

# 2. 确认部署的 commit 是 main 最新 HEAD
git rev-parse origin/main  # 本地 main HEAD
# 与服务器 deployed-release.json 的 releaseId 对比

# 3. 确认前端 hash 已更新（不是旧版本）
curl -s http://${TARGET_HOST}:8080/ | grep -oE 'assets/index-[A-Za-z0-9_-]+\.js'
# 与本地打包的 .release/<release-id>/frontend/index.html 对比
```

### 相关文档

- 第 114 次测试环境部署（基线 `268736e12`，**不含 PR !2227**，是本次部署遗漏的源头；报告未合入 main，仅本地存档后清理）
- 第 86 条：招标文件 PDF 关键词识别陷阱（复合 bug 组成部分 1：归一化字符集）
- 第 87 条：结构化标签字段死磕 Prompt 不如正则兜底（复合 bug 组成部分 2：正则兜底）
- 第 88 条：持久化层"只在空值时更新"策略（复合 bug 组成部分 4：持久化层）
- 第 89 条：前端字段映射缺 purchaserName（复合 bug 组成部分 5：前端 mapping）
...
**背景**：T034 quickstart.md §4 E2E 测试 9 个全部失败，但 CO-601 产品代码经手动 API 验证完全正常。E2E 失败不等于产品代码有 bug，必须区分测试代码问题和产品代码缺陷。

**三类根因模式（均为测试代码/环境问题）**：

1. **测试数据污染（最常见）**：测试运行前未重置表单定义，累积残留字段导致后续 PUT 校验报 "字段 key 重复"。
   - 根因：测试代码 `beforeEach` 只创建新字段，未清理上一次运行的残留
   - 修复方向：`beforeEach` 先 DELETE 表单定义或用唯一 scope 隔离
   - 检查信号：连续两次运行同一测试，第二次开始报 "key 重复"

2. **角色权限不匹配**：测试用 `bid-Team` 角色创建项目，但 `POST /api/projects` 要求 `ADMIN/MANAGER`，被 `Access Denied`。
   - 根因：测试代码选错角色，未对照 `@PreAuthorize` 注解
   - 修复方向：测试前查 `@PreAuthorize` 确定所需角色，用 `/bidAdmin` 等有权限的角色
   - 检查信号：API 返回 403 Access Denied，但手动用 admin 调用同接口 200

3. **后端 OOM 崩溃（环境问题）**：E2E 大量请求触发后端内存增长，`exit code: 137`（SIGKILL），后续测试全部 `ECONNREFUSED`。
   - 根因：后端默认堆大小不足，E2E 并发请求 + 大对象未释放
   - 修复方向：启动加 `-Xmx2g`；E2E 测试间清理大对象；后端排查内存泄漏
