...
| 功能 | 投标管理员 | 投标组长 | 投标项目负责人 | 投标专员 |
|---|---|---|---|---|
| 查看标讯详情 | ✅ 全量 | ✅ 全量 | ✅ 仅自己的 | ✅ 仅分配给自己的 |
| 关键字/目标客户清单配置 | — | — | — | — |
| 标讯去重·冲突拍板 | ✅ 最终决定 | 提报给管理员 | — | — |
| 查看分发变更日志 | ✅ | ✅ | — | — |
| 弃标/未中标后仍可查看列表 | ✅ | ✅ | ✅ | ✅ |
...
| 文档功能 | 端点 | 代码实现 | 文档对照 |
|---|---|---|---|
| 查看详情 | GET `/{id}` | 类级 hasAnyRole 6 角色 + Service `assertCanAccessTender` | ✅ 角色覆盖 + 数据范围（同 2.1）|
| 查看分发变更日志 | GET `/{id}/audit-logs` | `hasAuthority('tender.view')` | ✅ 匹配（tender.view 被 admin/bidAdmin/bid-TeamLeader 持有，不含 sales/专员）|
| 关键字/客户清单配置 | ❌ 无端点 | — | ❓ 文档"—"（业务规则第 8 条：系统管理员初始化导入，走运维）|
| 标讯去重·冲突拍板 | ❌ 无拍板端点 | 仅创建时自动检测（`TenderDeduplicationService`）| ❌ **缺失**（文档要求管理员"最终决定"入口 + 组长"提报"入口）|
| 弃标/未中标后仍可查看列表 | 同 2.1 列表 | `filterVisibleTenders` 不按状态过滤 | ✅ 匹配（状态不影响可见性）|

### 差距判断

| 维度 | 结论 |
|---|---|
| 详情查看角色 + 数据范围 | ✅ 匹配（Service 层过滤，同 2.1）|
| 审计日志角色收口 | ✅ 匹配（tender.view 仅管理员/组长，项目负责人/专员被拒）|
| 弃标后仍可查看 | ✅ 匹配（filterVisibleTenders 不过滤状态）|
| **去重拍板入口** | ❌ **缺失**——文档要求管理员最终决定/组长提报，代码只有创建时硬拦截 |
| 关键字/客户清单配置 | ❓ 文档"—"（运维流程，非系统功能）|

### 契约测试

**Controller 层集成测试**（`TenderPermissionIntegrationTest`，+4 = 25/25）：
- 详情 GET：项目负责人非 403（Service 层校验可见性）
- 审计日志 GET：项目负责人 403（tender.view 不含）
- 审计日志 GET：投标专员 403
- 审计日志 GET：投标组长（authorities=tender.view）非 403

### ⚠️ 待业务确认（去重拍板）

文档要求"标讯去重·冲突拍板：投标管理员最终决定，组长提报"。当前代码 `TenderDeduplicationService` 只在创建时自动检测重复并硬拦截（抛 `TenderDuplicateException`），**没有管理员的"最终决定保留"入口，也没有组长的"提报"入口**。

需业务确认：
- 是否需要补一个"管理员强制保留重复标讯"的端点？（如 POST `/api/tenders/{id}/force-keep`）
- 组长"提报"是系统内功能还是线下沟通？

按当前实现，重复标讯无法创建（硬拦截），可能是简化设计。**不擅自补端点**——待业务确认。

### 2.4 小结

详情查看、审计日志、弃标后可见性都匹配文档。**唯一缺口是"去重拍板"功能未实现**（文档要求的角色化决策入口）。其余补充功能正确并已锁定。
