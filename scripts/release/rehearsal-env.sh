...
# H13 cookie 注入需要：auth-helpers.js 用 PLAYWRIGHT_BASE_URL 决定前端 cookie 域，
# 必须和 UAT_WEB_BASE_URL 一致，否则浏览器拿不到 access_token cookie 就被路由守卫跳 /login
PLAYWRIGHT_BASE_URL="${PLAYWRIGHT_BASE_URL:-$UAT_WEB_BASE_URL}"
...
export PLATFORM_ENCRYPTION_KEY UAT_TEST_PASSWORD ADMIN_PASSWORD UAT_API_BASE_URL UAT_WEB_BASE_URL PLAYWRIGHT_API_BASE_URL PLAYWRIGHT_BASE_URL
