...
start_backend() {
  ...
  # Auto-run Flyway repair + target clean before start (solves recurring post-sync/rebase Flyway issues)
  if [[ -x "$ROOT_DIR/scripts/dev-flyway-repair.sh" ]]; then
    echo "[backend] running dev-flyway-repair (auto, --repair-only) ..."
    XIYU_DEV_CONFIRMED=1 bash "$ROOT_DIR/scripts/dev-flyway-repair.sh" --repair-only || echo "[backend] repair script non-fatal error, continuing"
  fi
  ...
  (
    ...
    nohup env \
      ...
      mvn spring-boot:run -Dmaven.test.skip=true -DskipTests -Dspring-boot.run.arguments="--server.port=${BACKEND_PORT} --spring.flyway.enabled=false" \
