...
- **分支命名**：
  - **Worktree 锚点分支**（agent/<name>-<purpose>，如 `agent/codex-init`、`agent/claude-init`）：各 Agent worktree 的常驻基线分支。**严禁直接在此分支上开发**（CI 门禁会拦截），**严禁删除**（本地或远端均不可删）。仅用于 worktree 锚定和多 Agent 间同步基线。
  - **任务开发分支**（`codex/`、`cursor/`、`feat/`、`fix/`、`chore/` 等前缀）：日常功能开发、Bug 修复、CI 修复等使用的临时分支。PR 合入后由 CI 自动清理删除。
...
- **严禁绕过**：任何 `git push --no-verify` / `git commit --no-verify` 已被 `scripts/git` 包装器在系统层面禁止（见 CLAUDE.md “5. Git 安全规则” + `npm run agent:git-safe-check`）。早操 sync-env、who-touches、agent-start-task 均已自激活保护。
- **自动合并**：1 个 required review 批准后，`.github/workflows/auto-enable-merge-on-approved.yml` 会自动为 PR 开启 GitHub auto-merge（--squash）。真正合并仍需所有门禁（agent-locks、line-budget、frontend/backend/e2e + strict）通过。合并后用早操 sync-env 让“下一个线”继续。详见 CLAUDE.md §6。
